...
@export var peak_wind_force: float = 160.0    # Zone1-3 极风
@export var descent_wind_force: float = 30.0  # Zone4 顺风下山
...
func _process(delta: float) -> void:
	...
	if px < DESCENT_START_X:
		...
		var resistance: float = 0.28 if input_dir * _wind_dir < 0.0 else 1.0
